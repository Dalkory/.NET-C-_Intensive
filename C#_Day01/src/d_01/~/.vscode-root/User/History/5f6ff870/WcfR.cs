...
var cashRegisters = store.CashRegisters;
while (store.IsOpen())
{
    foreach (var customer in customers)
    {
        customer.FillCart(7);
        var cashRegister = CustomerExtensions.FewCustomers(cashRegisters);
...
while (store.IsOpen())
{
    ...
    {
        customer.FillCart(7);
        var cashRegister = CustomerExtensions.FewItems(cashRegisters);
        Console.WriteLine("{0}, Customer #{1} ({2} items in cart) - {3} ({4} people with {5} items behind)",
            customer.Name, 
            customer.Id, 
            customer.ItemsAmount, 
