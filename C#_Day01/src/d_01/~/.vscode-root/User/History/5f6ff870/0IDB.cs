...
{
    customers[index] = new Customer($"Customer {index}", index);
}
...
void ProcessCustomers(Store store, Func<List<CashRegister>, CashRegister> cashRegisterSelector)
{
    while (store.IsOpen())
    {
        foreach (var customer in customers)
        {
            ...
            {
                Console.WriteLine($"{customer.Name}, Customer #{customer.Id} ({customer.ItemsAmount} items in cart) - {cashRegister.Name} ({cashRegister.CustomersCount} people with {cashRegister.ItemsCount} items behind)");
