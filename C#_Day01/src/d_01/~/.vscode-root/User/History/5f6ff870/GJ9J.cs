...
List<CashRegister> cashRegistersFewCustomers = storeFewCustomers.CashRegisters;
while (storeFewCustomers.IsOpen())
{
    if (cashRegistersFewCustomers != null && cashRegistersFewCustomers.Count != 0)
    {
        foreach (var customer in customers)
        {
            customer.FillCart(7);
            CashRegister cashRegisterFewCustomers = CustomerExtensions.FewCustomers(cashRegistersFewCustomers);
            if (cashRegisterFewCustomers != null) {
                Console.WriteLine("{0}, Customer #{1} ({2} items in cart) - {3} ({4} people with {5} items behind)",
                    customer.Name, 
                    customer.Id, 
                    customer.ItemsAmount, 
                    cashRegisterFewCustomers.Name, 
                    cashRegisterFewCustomers.CustomersCount, 
                    cashRegisterFewCustomers.ItemsCount);
                cashRegisterFewCustomers.EnqueueCustomer(customer);
            } else {
                Console.WriteLine("No available cash register for customer {0}", customer.Name);
            }
        }
        storeFewCustomers.BuyAndLeave();
    } else {
        Console.WriteLine("No available cash registers");
        break;  // Exit the loop if there are no available cash registers
    }
}

Console.WriteLine("-------Few Items--------");
Store storeFewItems = new Store(40, 3);
List<CashRegister> cashRegistersFewItems = storeFewItems.CashRegisters;
while (storeFewItems.IsOpen())
{
    foreach (var customer in customers)
    {
        customer.FillCart(7);
        CashRegister cashRegisterFewItems = CustomerExtensions.FewItems(cashRegistersFewItems);
        Console.WriteLine("{0}, Customer #{1} ({2} items in cart) - {3} ({4} people with {5} items behind)",
            customer.Name, 
            customer.Id, 
            customer.ItemsAmount, 
            cashRegisterFewItems.Name,
            cashRegisterFewItems.CustomersCount, 
            cashRegisterFewItems.ItemsCount);
        cashRegisterFewItems.EnqueueCustomer(customer);
    }
    storeFewItems.BuyAndLeave();
}
