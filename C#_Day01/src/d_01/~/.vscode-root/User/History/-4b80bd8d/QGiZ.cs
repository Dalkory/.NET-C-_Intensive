        ...
        public void BuyAndLeave()
        {
            foreach (var cashRegister in _CashRegisters)
            {
                if (cashRegister != null) 
                {
                    ...
                    while (customer != null)
                    {
                        if (_storage != null && customer != null && customer.ItemsAmount.HasValue) {
                            customer.Buy(_storage.GetItems(customer.ItemsAmount.Value));
                            if (customer.ItemsAmount.HasValue && customer.ItemsAmount.Value != 0)
                            {
